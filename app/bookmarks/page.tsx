...
export default function BookmarksPage() {
  const { user, preferences, toggleBookmark } = useAuth()

  if (!user) {
    return (
      <div className="min-h-screen bg-gray-950 text-white flex items-center justify-center">
        <div className="text-center">
          <p className="text-xl mb-4">Please sign in to view your bookmarks</p>
          <Link href="/">
  ...
  const bookmarkedManhwa = manhwaData.filter((manhwa) => preferences.bookmarks.includes(manhwa.id))
      ...
      <main className="container mx-auto px-4 py-6">
        {bookmarkedManhwa.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-400 mb-6">Bookmark manhwa to read them later</p>
            <Link href="/">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
            {bookmarkedManhwa.map((manhwa) => (
              <ManhwaCard key={manhwa.id} manhwa={manhwa} onRemoveBookmark={toggleBookmark} />
            ))}
          </div>
        )}
      </main>
...
function ManhwaCard({ manhwa, onRemoveBookmark }: { manhwa: any; onRemoveBookmark: (id: number) => void }) {
  return (
